...
출장 전 차량 확인과 제품·총비용 안내를 부탁드립니다.${photos.length?'\n※ 문자 앱에서 첨부할 사진: '+photos.join(', '):''}`;
  ...
  return <form id="sms-inquiry" className="sms-form" onSubmit={e=>{e.preventDefault();send();}} onChange={()=>setStatus('')}>
    ...
    <fieldset className="sms-fieldset"><legend>증상 <span>해당하는 항목에 체크</span></legend><div className="check-chips">{['시동이 안 걸려요','방전이 반복돼요','경고등이 켜졌어요','미리 교체하고 싶어요','잘 모르겠어요'].map(s=><label key={s} className={symptoms.includes(s)?'selected':''}><input type="checkbox" checked={symptoms.includes(s)} onChange={()=>toggleSymptom(s)}/><span>{s}</span></label>)}</div></fieldset>
